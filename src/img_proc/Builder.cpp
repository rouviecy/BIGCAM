...
void Builder::Build(){
	for(size_t i = 0; i < list_img.size(); i++){
		...
		mat_transl.at<float>(0, 0) = (float) 1;
		mat_transl.at<float>(0, 1) = (float) 0;
		mat_transl.at<float>(0, 2) = (float) list_img[i].img.cols / 2;
		mat_transl.at<float>(1, 0) = (float) 0;
		mat_transl.at<float>(1, 1) = (float) 1;
		mat_transl.at<float>(1, 2) = (float) list_img[i].img.rows / 2;
		vector <cv::Point> vertices;
		vertices.push_back(cv::Point(0,						0));
		vertices.push_back(cv::Point(list_img[i].img.rows,	0));
		vertices.push_back(cv::Point(list_img[i].img.rows,	list_img[i].img.cols));
		vertices.push_back(cv::Point(0,						list_img[i].img.cols));
		list_img[i].rotation_rect.center = img_center;
		list_img[i].rotation_rect.angle = list_img[i].thz * 57.296;
		list_img[i].rotation_rect.size = cv::Size(list_img[i].img.cols, list_img[i].img.rows);
		cv::Size new_size(list_img[i].img.cols * 2, list_img[i].img.rows * 2);
		cv::Mat tempo = cv::Mat::zeros(new_size, CV_8UC3);
	...
	for(size_t i = 0; i < list_img.size(); i++){
		...
		cv::Point vertice_A(list_img[i].x_map - x_offset + rect_vertices[0].x, list_img[i].y_map - y_offset + rect_vertices[0].y);
		cv::Point vertice_B(list_img[i].x_map - x_offset + rect_vertices[1].x, list_img[i].y_map - y_offset + rect_vertices[1].y);
		cv::Point vertice_C(list_img[i].x_map - x_offset + rect_vertices[2].x, list_img[i].y_map - y_offset + rect_vertices[2].y);
		cv::Point vertice_D(list_img[i].x_map - x_offset + rect_vertices[3].x, list_img[i].y_map - y_offset + rect_vertices[3].y);
		poly_vertices.push_back(vertice_A);
		poly_vertices.push_back(vertice_B);
		poly_vertices.push_back(vertice_C);
		poly_vertices.push_back(vertice_D);
