...
void Monitor::Draw_monitor(){
	...
	for(size_t i = 0; i < path.size(); i++){
		int draw_x = MONITOR_BORDER / 2 + (int) ((float) (MONITOR_SIZE - MONITOR_BORDER) * ((float) path[i][0] - x_min) / (x_max - x_min));
		int draw_y = MONITOR_BORDER / 2 + (int) ((float) (MONITOR_SIZE - MONITOR_BORDER) * ((float) path[i][1] - y_min) / (y_max - y_min));
